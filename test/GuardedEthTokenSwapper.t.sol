...
contract GuardedEthTokenSwapperTest is Test {
    ...
    function _setupAllFeeds() internal {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            if (isForkMode) {
                address pool =
                    IUniswapV3Factory(UNISWAP_V3_FACTORY).getPool(WETH, tokenConfigs[i].token, tokenConfigs[i].feeTier);
    ...
    function testSwapEthForToken() public {
        ...
        uint256 amountOut = swapper.swapEthForToken{value: ethAmount}(token, slippage, deadline);
    ...
    function testSwapMultipleEthPairs() public {
        ...
        for (uint256 i = 0; i < tokenConfigs.length; i++) {
            ...
            uint256 amountOut;
            {
                ...
                uint256 expectedTokens = calculateExpectedTokens(
                    ethAmount, uint256(oraclePrice), oracleDecimals, tokenDecimals, feedDescription
                );
                ...
                // Execute the swap
                amountOut = swapper.swapEthForToken{value: ethAmount}(token, slippage, deadline);
        ...
        assertEq(
            successfulValidations,
            tokenConfigs.length,
            "All tokens must pass oracle validation (100% success rate required)"
        );
    ...
    function validateSingleTokenSwap(TokenConfig memory config, uint256 ethAmount, uint16 slippage, uint256 deadline)
        external
    {
        ...
        uint256 expectedTokens =
            (ethAmount * uint256(oraclePrice) * (10 ** tokenDecimals)) / (1e18 * (10 ** oracleDecimals));
    ...
    function testLiquidityValidation() public {
        ...
        assertEq(
            successfulSwaps,
            tokenConfigs.length,
            "All tokens must pass liquidity validation (100% success rate required)"
        );
    ...
    function testZeroEthRevert() public {
        ...
        swapper.swapEthForToken{value: 0}(LINK, 200, deadline);
    ...
    function testFeedNotSet() public {
        ...
        swapper.swapEthForToken{value: 1 ether}(randomToken, 200, deadline);
    ...
    function testSimplifiedEvents() public {
        ...
        swapper.swapEthForToken{value: ethAmount}(LINK, 200, deadline);
